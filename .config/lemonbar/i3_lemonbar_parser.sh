  ...
  case $line in
    SYS*)
      ...
      # eth
        ...
        ethd="%{F${eth_cback}}${sep_left}%{F B${eth_cback}} %{F${eth_cfore} T1} ${ethd_v}"
        ethu="%{F${eth_cicon}}${sep_l_left} %{T2}"BAT"%{F${eth_cfore} T1} ${ethu_v}"
      ;;
    VOL*)
      # Volume
      vol="%{F${color_sec_b2}}${sep_left}%{F${color_icon} B${color_sec_b2}} %{T2}${icon_vol}%{F- T1} ${line#VOL}"
      ;;
    GMA*)
      # Gmail
      gmail="${line#???}"
    ...
    WIN*)
      ...
      title="%{F${color_head} B${color_sec_b3}}${sep_right}%{F${color_head} B${color_sec_b3} T2} ${icon_prog} %{F${color_sec_b3} B-}${sep_right}%{F- B- T1} %{F${color_wsptext}}${title}"
      ;;
  esac
